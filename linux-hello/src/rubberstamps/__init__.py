...
class RubberStamp:
	"""Linux Hello rubber stamp"""

	UI_TEXT = "ui_text"
	UI_SUBTEXT = "ui_subtext"

	def set_ui_text(self, text, type=None):
		"""Convert an ui string to input the auth UI understands"""
		typedec = "M"
	...
	def send_ui_raw(self, command):
		"""Write raw command to the auth UI stdin"""
		if self.config.getboolean("debug", "verbose_stamps", fallback=False):
			print("Sending command to auth UI: " + command)
